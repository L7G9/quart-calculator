...
from calculator_service.views.calculator_blueprints import PARAM_1_KEY, PARAM_2_KEY, LIST_KEY, RESULT_KEY
...
class TestApp(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_data(self):
        ...
        result = await app.get("/add", headers = self.headers, data = json.dumps(self.ab_data))
    ...
    async def test_subtract_data(self):
        app = tested_app.test_client()
        result = await app.get("/subtract", headers = self.headers, data = json.dumps(self.ab_data))
        body = json.loads(str(await result.get_data(), "utf8"))
    ...
    async def test_multiply_data(self):
        app = tested_app.test_client()
        result = await app.get("/multiply", headers = self.headers, data = json.dumps(self.ab_data))
        body = json.loads(str(await result.get_data(), "utf8"))
    ...
    async def test_divide_data(self):
        app = tested_app.test_client()
        result = await app.get("/divide", headers = self.headers, data = json.dumps(self.ab_data))
        body = json.loads(str(await result.get_data(), "utf8"))
    ...
    async def test_sum(self):
        app = tested_app.test_client()
        result = await app.get("/sum", headers = self.headers, data = json.dumps(self.list_data))
        body = json.loads(str(await result.get_data(), "utf8"))
        self.assertEqual(body[RESULT_KEY], 15)

    async def test_mean(self):
        app = tested_app.test_client()
        result = await app.get("/mean", headers = self.headers, data = json.dumps(self.list_data))
        body = json.loads(str(await result.get_data(), "utf8"))
        self.assertEqual(body[RESULT_KEY], 3)
